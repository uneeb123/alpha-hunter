...
async function testVectorStore() {
  ...
  if (searchResults.length > 0) {
    ...
    console.log(
      `Page content: ${searchResults[0].pageContent.substring(0, 100)}...`,
    );
  ...
  console.log(
    `similaritySearch with empty query found ${emptyResults.length} results`,
  );

  if (emptyResults.length > 0) {
    console.log('Sample result:');
    console.log(
      `Page content: ${emptyResults[0].pageContent.substring(0, 100)}...`,
    );
  ...
  try {
    ...
    console.log(
      `After adding: similaritySearch found ${postAddResults.length} results`,
    );

    // Try getRecentNewsHeadlines
    const postAddHeadlines = await vectorStore.getRecentNewsHeadlines(5);
    console.log(
      `After adding: getRecentNewsHeadlines found ${postAddHeadlines.length} headlines`,
    );
