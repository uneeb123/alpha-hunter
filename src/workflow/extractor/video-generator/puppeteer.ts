...
interface VideoConfig
  extends Required<Omit<VideoOptions, 'duration' | 'colorKeyframes'>> {
  duration: number | null;
  colorKeyframes: ColorKeyframe[];
}
...
async function generateVideo(
  ...
  // Use regex to replace placeholders to avoid issues with template literals
  ...
  htmlTemplate = htmlTemplate.replace(/\{\{RED\}\}/g, config.red.toString());
  htmlTemplate = htmlTemplate.replace(
    /\{\{GREEN\}\}/g,
    config.green.toString(),
  );
  htmlTemplate = htmlTemplate.replace(/\{\{BLUE\}\}/g, config.blue.toString());
  htmlTemplate = htmlTemplate.replace(
    /\{\{THRESHOLD\}\}/g,
  ...
  // Convert color keyframes array to string
  const colorKeyframesString = JSON.stringify(config.colorKeyframes, null, 4);
  htmlTemplate = htmlTemplate.replace(
    /\{\{COLOR_KEYFRAMES\}\}/g,
    colorKeyframesString,
  );
  ...
  try {
    ...
    while (Date.now() < endTime) {
      ...
      if (frameCount % 30 === 0) {
        ...
        console.log(
          `Captured ${frameCount} frames in ${elapsedSec.toFixed(
            1,
